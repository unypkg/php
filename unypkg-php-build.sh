...
unyp install re2c icu/68 curl libpng libwebp libjpeg-turbo freetype libgd imagemagick \
    pcre2 libxml2 libxslt libexif libzip oniguruma argon2 openssl/1.1.1w
...
./configure \
    ...
    --with-pdo-mysql \
    --enable-ctype \
    --with-openssl=/uny/pkg/openssl/1.1.1w \
    --with-curl \
    --enable-exif \
    --enable-mbstring \
    --with-zip \
